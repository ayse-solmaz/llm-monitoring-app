# Upstream issue — MLC Gemma tied-embed int4 stop-token pathology

> Target: https://github.com/mlc-ai/mlc-llm/issues  
> Do **not** attach private LoRA/weights; public base **Gemma-2B-IT** + repro steps are enough.

**Status 2026-08-05:** Root cause confirmed as **`DIFFUSE_WINNER_ROW`** (int4 tied-embed rows inflate false winners). Fix validated: scheme **`q4f16_2`** (`quantize_embedding=False`, `quantize_final_fc=False`) restores margin107=0 and clean stop. **Issue not yet filed** — `gh auth login` required (see [Filing](#filing) below).

Related local docs: [MLC_STATUS_ANALYSIS.md](./MLC_STATUS_ANALYSIS.md), [MLC_Q4_LMHEAD_HUNT.md](./MLC_Q4_LMHEAD_HUNT.md).

---

## Title

**Gemma-2B-IT tied-embed int4 (`quantize_embedding=True`) inflates false-winner rows (~+13 logit) so `<end_of_turn>` (107) loses by ~1.8 at stop step; `q4f16_2` (float embed) fixes**

---

## Issue body (paste or `gh issue create`)
...
**Blocker (2026-08-05):** `gh auth status` → not logged in. Run `gh auth login`, then:

```powershell
gh issue create --repo mlc-ai/mlc-llm `
  --title "Gemma-2B-IT tied-embed int4 (quantize_embedding=True) inflates false-winner rows (~+13 logit) so <end_of_turn> (107) loses by ~1.8 at stop step; q4f16_2 (float embed) fixes" `
  --body-file docs/MLC_UPSTREAM_ISSUE_BODY.md
```

After filing, set **Issue URL:** below and in [MLC_STATUS_ANALYSIS.md](./MLC_STATUS_ANALYSIS.md) §9 if desired.

**Issue URL:** *(pending — run command above)*

---

## Product note (local only — do not paste into GitHub)

Prod stays on base q4f16_1. FT Option A validated on **diag** `:8088` only. Knowledge/coverage (Problem B) is separate from this stop-cliff.
