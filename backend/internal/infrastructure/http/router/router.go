...
import (
	...
	appcompatHandler "github.com/masterfabric-go/masterfabric/internal/infrastructure/http/handler/appcompat"
	appconfigHandler "github.com/masterfabric-go/masterfabric/internal/infrastructure/http/handler/appconfig"
	appcmnHandler "github.com/masterfabric-go/masterfabric/internal/infrastructure/http/handler/appcmn"
...
func New(deps Dependencies) *chi.Mux {
	...
	r.Route("/api/v1", func(r chi.Router) {
		if deps.CMNHandler != nil {
			r.Get("/healthz", deps.CMNHandler.Healthz)
			r.Get("/version", deps.CMNHandler.Version)
		}
		if deps.ConfigHandler != nil {
			r.Get("/config", deps.ConfigHandler.GetConfig)
			r.Get("/config/models", deps.ConfigHandler.GetModels)
		}
		...
		r.Group(func(r chi.Router) {
			...
			if deps.LLMHandler != nil {
				r.Route("/llm", func(r chi.Router) {
					...
					r.Get("/sessions", deps.LLMHandler.ListSessions)
					r.Get("/sessions/{id}", deps.LLMHandler.GetSession)
					r.Delete("/sessions/{id}", deps.LLMHandler.DeleteSession)
					r.Post("/sessions/{id}/messages", deps.LLMHandler.CreateMessage)
