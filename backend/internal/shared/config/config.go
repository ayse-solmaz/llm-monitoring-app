...
// ConnectionString returns DATABASE_URL when set, otherwise a constructed DSN.
...
func Load() *Config {
	corsOrigins := envOrDefaultSlice("CORS_ALLOWED_ORIGINS", nil)
	...
	return &Config{
		...
		WebSocket: WebSocketConfig{
			Enabled:         envOrDefault("WS_ENABLED", "false") == "true",
